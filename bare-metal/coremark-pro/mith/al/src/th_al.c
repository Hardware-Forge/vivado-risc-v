...
#endif
// Custom implementation for bare-metal RISC-V using mcycle CSR
...
static unsigned long start_cycles;
...
#endif
	al_power_finished();
    unsigned long end_cycles = read_cycles();
#if REPORT_THMALLOC_STATS
	th_printf("Total allocated memory at signal finish: %.02fMB\n",(e_f32)th_malloc_total/(1024.0*1024.0));
#endif
	return (size_t)(end_cycles - start_cycles);
}

size_t al_signal_now( void )
{
    unsigned long now = read_cycles();
	return (size_t)(now - start_cycles);
}
...
{
#if HOST_EXAMPLE_CODE 
    /* For bare-metal FPGA with rdcycle, return actual CPU frequency in Hz */
    /* FPGA_CPU_CLK_FREQ is defined in MHz, so multiply by 1,000,000 */
    return (size_t) (FPGA_CPU_CLK_FREQ * 1000000UL);
#else
