...
/* Global to export score to harness */
...
int mith_report_results(ee_workload *workload, size_t total_time, e_u32 num_contexts, unsigned int num_workers) {
	...
	g_last_score = (double)workload->iterations/secs;
	mith_log_dbl(wname,"workloads/sec",g_last_score);
#endif
#if BMDEBUG
