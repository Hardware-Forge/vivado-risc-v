...
*/
char	*th_getenv(const char *key) {return NULL;}
/* Override libc getenv */
...
void redirect_std_files() {
	th_stdin=NULL; //stdin;
	th_stdout=NULL; //stdout;
	th_stderr=NULL; //stderr;
}
...
/* Function: th_strtod
	string to double.
*/
/* Override libc strtod to prevent pulling in relocation-heavy code */
/* Override libc strtod to prevent pulling in relocation-heavy code */
double strtod(const char *nptr, char **endptr) {
    ...
    if (*p == 'e' || *p == 'E') {
        ...
        // compute power
        double power = 1.0;
        double base_pow = 10.0;
        while (exp > 0) {
            if (exp & 1) power *= base_pow;
            base_pow *= base_pow;
            exp >>= 1;
        }
        if (exp_sign == 1) res *= power;
        else res /= power;
    }
...
double _strtod_r(struct _reent *reent, const char *nptr, char **endptr) { return strtod(nptr, endptr); }
/* Function: th_atof
	string to double.
