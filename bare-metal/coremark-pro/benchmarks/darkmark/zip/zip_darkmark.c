...
static char *gen_parse_buf(e_u32 *psize, void *r, e_u32 record_type) {
	...
	*buf=0;
	...
	/* setup xml/html header */
	th_strcpy(buf,default_start);
    char *p = buf + th_strlen(buf); /* optimization: track end of string */
	...
	while (size > (end_size+max_entry)) 
	{
        ...
        /* Optimized append: O(N) instead of O(N^2) */
        th_strcpy(p, lbuf);
        int llen = th_strlen(lbuf);
        p += llen;
		size-=llen;
	}
	/* setup xml/html footer */
	th_strcpy(p,default_end);
	size-=end_size;
