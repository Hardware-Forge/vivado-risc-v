...
// ============================================================================
// BOP Prefetcher Benchmark
// Tests offset learning capabilities
// ============================================================================
...
// Generic stride test
void run_test_offset(int offset, const char* name) {
    printf("Starting %s test (offset=%d blocks)...\n", name, offset);
    configure_l1_events();
    
    uint64_t sum = 0;
    l2_counters_t l2_start, l2_end;
    l1_counters_t l1_start, l1_end;
    int stride = offset * CACHE_LINE;
...
// Test changing offset mid-stream
void run_test_changing_offset(const char* name) {
    ...
    // First half: offset +1
    for (int i = 0; i < BUFFER_SIZE / 2; i += CACHE_LINE) {
        sum += buffer[i];
    }
    // Second half: offset +4
    for (int i = BUFFER_SIZE / 2; i < BUFFER_SIZE; i += 4 * CACHE_LINE) {
        sum += buffer[i];
    }
    ...
    uint64_t accesses = (BUFFER_SIZE / 2) / CACHE_LINE + (BUFFER_SIZE / 2) / (4 * CACHE_LINE);
    printf("%s Results:\n", name);
    printf("  Total Cycles: %lu\n", end - start);
    printf("  Accesses:     %lu\n", accesses);
    printf("  Cycles/Access: %lu\n", (end - start) / accesses);
...
int main(void) {
    printf("BOP Prefetcher Benchmark\n");
    printf("Buffer Size: %d KB\n", BUFFER_SIZE / 1024);
    
    // Test various offsets - BOP should find the best one
    run_test_offset(1, "Offset +1");
    run_test_offset(4, "Offset +4");
    run_test_offset(8, "Offset +8");
    run_test_changing_offset("Changing Offset");
