...
/**
 * Backwards-compatible key expected by some external packages (e.g. sifive inclusivecache).
 * Some code references `SubsystemBankedCoherenceKey` to query `nBanks` and `coherenceManager`.
 * Provide a default `BankedL2Params` so those references resolve.
 */
case object SubsystemBankedCoherenceKey extends Field(BankedL2Params())
