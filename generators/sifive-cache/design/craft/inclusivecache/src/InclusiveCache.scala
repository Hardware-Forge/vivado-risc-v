    ...
    extends LazyModule
{
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    // Global performance counters (aggregate over all banks)
    val l2Hits      = RegInit(0.U(64.W))
    val l2Misses    = RegInit(0.U(64.W))
    val mshrHits    = RegInit(0.U(64.W))
    val mshrMisses  = RegInit(0.U(64.W))
    ...
    val mods = (node.in zip node.out) map { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      // Accumulate per-bank perf pulses into global 64-bit counters
      when (scheduler.io.perf_l2_hit)    { l2Hits     := l2Hits     + 1.U }
      when (scheduler.io.perf_l2_miss)   { l2Misses   := l2Misses   + 1.U }
      when (scheduler.io.perf_mshr_hit)  { mshrHits   := mshrHits   + 1.U }
      when (scheduler.io.perf_mshr_alloc){ mshrMisses := mshrMisses + 1.U }
    ...
    // Drive perf counter values into each control block (if present)
    ctrls.foreach { ctrl =>
      ctrl.module.io.l2_hits      := l2Hits
      ctrl.module.io.l2_misses    := l2Misses
      ctrl.module.io.mshr_hits    := mshrHits
      ctrl.module.io.mshr_misses  := mshrMisses
    }
