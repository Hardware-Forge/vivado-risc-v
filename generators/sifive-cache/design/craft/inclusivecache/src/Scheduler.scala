...
class InclusiveCacheBankScheduler(params: InclusiveCacheParameters) extends Module
{
  val io = IO(new Bundle {
    ...
    // Performance event pulses (one-cycle, per bank)
    val perf_l2_hit    = Output(Bool())
    val perf_l2_miss   = Output(Bool())
    val perf_mshr_hit  = Output(Bool())
    // mshr_alloc: primary L2 misses / MSHR allocations
    val perf_mshr_alloc= Output(Bool())
  })
  ...
  // L2 directory hit / miss (per lookup that completes)
  val dir_fire = directory.io.result.valid
  io.perf_l2_hit  := dir_fire && directory.io.result.bits.hit
  io.perf_l2_miss := dir_fire && !directory.io.result.bits.hit

  // MSHR miss = allocate a new MSHR for this request (no existing same-set owner)
  // MSHR hit  = queue secondary request to existing MSHR (same-set owner)
  val req_fire = request.valid && request.ready
  val mshr_alloc_pulse = req_fire && alloc && !mshr_uses_directory_assuming_no_bypass
  io.perf_mshr_alloc := mshr_alloc_pulse
  io.perf_mshr_hit   := req_fire && queue
