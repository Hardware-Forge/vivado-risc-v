...
class InclusiveCacheControl(outer: InclusiveCache, control: InclusiveCacheControlParameters)(implicit p: Parameters) extends LazyModule()(p) {
  ...
  class Impl extends LazyModuleImp(this) {
    val io = IO(new Bundle {
      ...
      // Read-only performance counters from the InclusiveCache
      val l2_hits      = Input(UInt(64.W))
      val l2_misses    = Input(UInt(64.W))
      val mshr_hits    = Input(UInt(64.W))
      val mshr_misses  = Input(UInt(64.W))
    })
    // Flush directive
    ...
    // Performance counter RegFields (read-only, 64-bit)
    val l2HitsR      = RegField.r(64, io.l2_hits,     RegFieldDesc("L2Hits",      "Number of L2 directory hits"))
    val l2MissesR    = RegField.r(64, io.l2_misses,   RegFieldDesc("L2Misses",    "Number of L2 directory misses"))
    val mshrHitsR    = RegField.r(64, io.mshr_hits,   RegFieldDesc("MSHRHits",    "Number of MSHR secondary hits (queued requests)"))
    val mshrMissesR  = RegField.r(64, io.mshr_misses, RegFieldDesc("MSHRMisses",  "Number of MSHR allocations (primary misses)"))
    // Stub prefetch counters (always 0 - SiFive cache has no L2 prefetcher)
    val pfIssuedR    = RegField.r(64, 0.U,            RegFieldDesc("PFIssued",    "Number of L2 Prefetches Issued (not supported)"))
    val pfUsedR      = RegField.r(64, 0.U,            RegFieldDesc("PFUsed",      "Number of L2 Prefetched Lines Used (not supported)"))
    ...
    val regmap = ctrlnode.regmap(
      ...
      // 0x100 - 0x12F: performance counters (all 64-bit)
      0x100 -> Seq(l2HitsR),
      0x108 -> Seq(l2MissesR),
      0x110 -> Seq(mshrMissesR),
      0x118 -> Seq(pfIssuedR),
      0x120 -> Seq(pfUsedR),
      0x128 -> Seq(mshrHitsR),
