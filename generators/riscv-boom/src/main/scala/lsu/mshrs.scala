  ...
  with HasL1HellaCacheParameters
{
  val io = IO(new Bundle {
    ...
    val sb_alloc   = Decoupled(new StreamBufferRequest) // Stream Buffer allocation port
    val wb_req     = Decoupled(new WritebackReq(edge.bundle))
  ...
  val prefetcher: DataPrefetcher = if (enablePrefetching) {
    if (boomParams.prefetchers.isEmpty) Module(new NullPrefetcher)
    else Module(new MultiDataPrefetcher(boomParams.prefetchers))
  } else Module(new NullPrefetcher)

  // Prefetch queue kept for old prefetch pipeline (disabled)
  val prefetch_q = Module(new Queue(new BoomDCacheReq, 4))
  prefetch_q.io.enq.valid := false.B  // Disabled - using direct SB path
  prefetch_q.io.enq.bits := DontCare
  prefetch_q.io.deq.ready := false.B
  ...
  io.prefetch.valid       := false.B  // Old prefetch path disabled
  io.prefetch.bits        := DontCare
  
  // Stream Buffer allocation: Connect DIRECTLY from prefetcher (bypass prefetch_q)
  io.sb_alloc.valid       := prefetcher.io.prefetch.valid
  io.sb_alloc.bits.address := prefetcher.io.prefetch.bits.addr
  prefetcher.io.prefetch.ready := io.sb_alloc.ready
  
  // Debug print
  when (prefetcher.io.prefetch.valid) {
    printf("[MSHR->SB] Prefetch direct to SB: addr=0x%x sb_ready=%d\n",
      prefetcher.io.prefetch.bits.addr, io.sb_alloc.ready)
  }

  prefetcher.io.mshr_avail    := prefetch_q.io.enq.ready
  // Trigger prefetcher on PRIMARY miss allocation (not commit) for timely, in-order training
  prefetcher.io.req_val       := pri_val && pri_rdy
  prefetcher.io.req_addr      := req.bits.addr
  prefetcher.io.req_coh       := req.bits.old_meta.coh
}
