  ...
  with HasBoomCoreParameters
{
  ...
  // LSU performance counters (no prefetcher in this configuration)
  // Store miss & MSHR allocation (primary L1 data cache misses)
  ...
  // No stream buffer / prefetch buffer in this configuration
  io.lsu.perf.sb_hit    := false.B

  // Expose MSHR allocations as the MSHR perf counter
  io.lsu.perf.mshr_alloc := mshr_alloc_pulse

  // Prefetching not implemented: keep prefetch-related counters at zero
  io.lsu.perf.pf_issued := false.B
  io.lsu.perf.pf_used   := false.B
