...
class BoomNonBlockingDCache(staticIdForMetadataUseOnly: Int)(implicit p: Parameters) extends LazyModule
{
  ...
  // Stream Buffer depth for prefetching
  val sbDepth = tileParams.core.asInstanceOf[BoomCoreParams].streamBufferEntries
  ...
  // Stream Buffer uses source IDs after MMIO
  protected def sbClientParameters = Seq(TLMasterParameters.v1(
    name          = s"Core ${staticIdForMetadataUseOnly} DCache StreamBuffer",
    sourceId      = IdRange(cfg.nMSHRs + 1 + cfg.nMMIOs, cfg.nMSHRs + 1 + cfg.nMMIOs + sbDepth),
    supportsProbe = TransferSizes(cfg.blockBytes, cfg.blockBytes)))

  val node = TLClientNode(Seq(TLMasterPortParameters.v1(
    cacheClientParameters ++ mmioClientParameters ++ sbClientParameters,
    minLatency = 1)))
  ...
  with HasBoomCoreParameters
{
  ...
  // Stream Buffer for prefetching (source IDs start after MSHR + WB + MMIO)
  val sbSourceIdBase = cfg.nMSHRs + 1 + cfg.nMMIOs
  val streamBuffer = Module(new StreamBuffer(depth = outer.sbDepth, sourceIdBase = sbSourceIdBase))
  streamBuffer.io.req.ready := false.B // Default
  streamBuffer.io.inval.valid := false.B
  streamBuffer.io.inval.bits := 0.U
  ...
  // Connect prefetcher to Stream Buffer (prefetch bypasses MSHR path via Stream Buffer)
  streamBuffer.io.alloc <> mshrs.io.sb_alloc
  mshrs.io.prefetch.ready := false.B // Disable old prefetch path to pipeline
  ...
  // 0 goes to MSHR refills, 1 goes to prober, 2 goes to Stream Buffer
  ...
  // 0 goes to pipeline, 1 goes to MSHR refills, 2 goes to Stream Buffer refills
  dataWriteArb.io.in(2) <> streamBuffer.io.refill
  ...
  // -------
  // Prefetcher
  // Disable MSHR Prefetch Pipeline Injection
  // We want prefetches to ONLY go to the Stream Buffer (Sidebar)
  val prefetch_fire = false.B // mshrs.io.prefetch.fire
  ...
  metaReadArb.io.in(5).valid              := false.B // mshrs.io.prefetch.valid
  ...
  // mshrs.io.prefetch.ready := metaReadArb.io.in(5).ready (Driven by StreamBuffer now)
  ...
  // Stream Buffer hit check on S2 miss (only for reads)
  // Find which pipe has a Read Miss
  val s2_read_miss_vec = (0 until memWidth).map { w => 
    s2_valid(w) && !s2_hit(w) && isRead(s2_req(w).uop.mem_cmd) && s2_type === t_lsu
  }
  val s2_read_miss_idx = PriorityEncoder(s2_read_miss_vec)
  val s2_any_read_miss = s2_read_miss_vec.reduce(_ || _)

  // Peek the address of the first read miss (if any)
  streamBuffer.io.peek_addr := s2_req(s2_read_miss_idx).addr

  // Check Hit / Pending
  // Note: Only the prioritized pipe gets the hit/pending signal
  val s2_sb_hit_val = streamBuffer.io.peek_hit && s2_any_read_miss
  val s2_sb_pending_val = streamBuffer.io.peek_pending && s2_any_read_miss
  
  // Map back to width (Mask)
  // Only the pipe that was selected (s2_read_miss_idx) gets the Hit signal
  val s2_sb_hit = (0 until memWidth).map(w => s2_sb_hit_val && (w.U === s2_read_miss_idx))
  val s2_sb_pending = (0 until memWidth).map(w => s2_sb_pending_val && (w.U === s2_read_miss_idx))
  
  // Trigger Stream Buffer pop on hit (initiates replay to cache)
  streamBuffer.io.pop_valid := s2_sb_hit.reduce(_ || _)

  // nack because of incoming probe OR Stream Buffer Hit/Pending (Needs Replay to wait for Refill)
  val s2_nack_hit_pre = RegNext(VecInit(s1_nack))
  val s2_nack_hit     = widthMap(w => s2_nack_hit_pre(w) || s2_sb_hit(w) || s2_sb_pending(w))
  ...
  // Stream Buffer logic moved up for s2_nack_hit dependency

  // Miss handling
  for (w <- 0 until memWidth) {
    // Debug Stuck Store
    when (s2_valid(w) && s2_req(w).addr === "x80002088".U) {
      printf("[DCACHE STORE DBG] hit=%d sb_hit=%d ready=%d valid=%d nack=%d nack_miss=%d\n", 
             s2_hit(w), s2_sb_hit(w), mshrs.io.req(w).ready, mshrs.io.req(w).valid, s2_send_nack(w), s2_nack_miss(w))
    }
    
    mshrs.io.req(w).valid := s2_valid(w)          &&
                            !s2_hit(w)            &&
                            !s2_sb_hit(w)         && // Don't allocate MSHR if Stream Buffer hit (reads only)
                            !s2_nack_hit(w)       &&
                            !s2_nack_victim(w)    &&
                            !s2_nack_data(w)      &&
                            !s2_nack_wb(w)        &&
  ...
  // TileLink A channel: Arbiter between MSHRs and Stream Buffer
  val acquireArb = Module(new Arbiter(new TLBundleA(edge.bundle), 2))
  acquireArb.io.in(0) <> mshrs.io.mem_acquire
  acquireArb.io.in(1) <> streamBuffer.io.req
  tl_out.a <> acquireArb.io.out
  ...
  // TileLink D channel: Route by source ID
  // Source IDs: 0 to nMSHRs-1 = MSHRs, nMSHRs = WB, nMSHRs+1 to nMSHRs+nMMIOs = MMIO, sbSourceIdBase+ = StreamBuffer
  val d_is_release_ack = tl_out.d.bits.source === cfg.nMSHRs.U
  val d_is_stream_buffer = tl_out.d.bits.source >= sbSourceIdBase.U && 
                           tl_out.d.bits.source < (sbSourceIdBase + outer.sbDepth).U
  
  streamBuffer.io.resp.valid := false.B
  streamBuffer.io.resp.bits  := tl_out.d.bits
  mshrs.io.mem_grant.valid   := false.B
  mshrs.io.mem_grant.bits    := DontCare
  
  when (d_is_release_ack) {
    tl_out.d.ready := true.B
  } .elsewhen (d_is_stream_buffer) {
    printf("[DCACHE D] StreamBuffer grant src=%d valid=%d ready=%d\n",
      tl_out.d.bits.source, tl_out.d.valid, streamBuffer.io.resp.ready)
    streamBuffer.io.resp.valid := tl_out.d.valid
    tl_out.d.ready := streamBuffer.io.resp.ready
  } .otherwise {
    mshrs.io.mem_grant <> tl_out.d
  }
  
  when (tl_out.d.valid) {
    printf("[DCACHE D] src=%d is_release_ack=%d is_sb=%d\n",
      tl_out.d.bits.source, d_is_release_ack, d_is_stream_buffer)
  }

  // Invalidate Stream Buffer on MSHR allocation OR Store Hit (prevents dual copy coherence issues)
  val mshr_alloc_fire = (0 until memWidth).map(w => mshrs.io.req(w).valid && mshrs.io.req(w).ready)
  val mshr_alloc_idx  = PriorityEncoder(mshr_alloc_fire)
  
  // Option 2: Disable SB Invalidation. Let both SB and MSHR fetch; L2 merges.
  // streamBuffer.io.inval.valid := mshr_alloc_fire.reduce(_ || _)
  // streamBuffer.io.inval.bits := s2_req(mshr_alloc_idx).addr

  dataWriteArb.io.in(1) <> mshrs.io.refill
  metaWriteArb.io.in(0) <> mshrs.io.meta_write
  metaWriteArb.io.in(2) <> streamBuffer.io.meta_write


  when (mshrs.io.refill.fire) {
    printf("[DCACHE] MSHR Refill addr=0x%x\n", mshrs.io.refill.bits.addr)
  }

  for (w <- 0 until memWidth) {
      when (!s2_valid(w) && s2_req(w).addr === "x80002088".U) {
         // Use a counter or probability to avoid spamming 
         // For now just print, user can use tail
         printf("[DCACHE] S2 Idle on addr=0x%x\n", s2_req(w).addr)
      }
  }

  // TileLink E channel: Arbiter for GrantAcks
  val finishArb = Module(new Arbiter(new TLBundleE(edge.bundle), 2))
  finishArb.io.in(0) <> mshrs.io.mem_finish
  finishArb.io.in(1) <> streamBuffer.io.ack_e
  tl_out.e <> finishArb.io.out
  ...
  // MSHR allocations (primary L1 data cache misses)
  val mshr_alloc_pulse = mshr_alloc_fire.reduce(_||_)
  io.lsu.perf.store_miss := mshr_alloc_pulse && isWrite(s2_req(mshr_alloc_idx).uop.mem_cmd)

  io.lsu.perf.d_hit      := s2_valid.zip(s2_hit).map{case (v,h) => v && h}.reduce(_||_)
  io.lsu.perf.sb_hit     := s2_sb_hit.reduce(_||_)

  // Expose MSHR allocations on the mshr_alloc perf line
  io.lsu.perf.mshr_alloc := mshr_alloc_pulse

  // L1 data prefetcher performance: issued vs used (via Stream Buffer)
  io.lsu.perf.pf_issued := streamBuffer.io.req.fire
  io.lsu.perf.pf_used   := s2_sb_hit.reduce(_||_)
