...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  val perf = Input(new Bundle {
    ...
    val d_hit     = Bool()
    val sb_hit    = Bool()
    // mshr_alloc: primary L1 data cache misses (MSHR allocations)
    val mshr_alloc= Bool()
    val pf_issued = Bool()
    val pf_used   = Bool()
  })
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  io.core.perf.store_miss := io.dmem.perf.store_miss

  // L1 Performance Counters
  io.core.perf.d_hit      := io.dmem.perf.d_hit
  io.core.perf.sb_hit     := io.dmem.perf.sb_hit
  io.core.perf.mshr_alloc := io.dmem.perf.mshr_alloc
  io.core.perf.pf_issued  := io.dmem.perf.pf_issued
  io.core.perf.pf_used    := io.dmem.perf.pf_used
